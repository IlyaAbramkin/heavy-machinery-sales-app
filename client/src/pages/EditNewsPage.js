...
const EditNewsPage = () => {
      ...
      try {
        ...
        setFormData({
          ...
          publication_date: newsData.publication_date 
            ? new Date(newsData.publication_date).toISOString().split('T')[0]
            : new Date().toISOString().split('T')[0]
        });
        
        // Если есть изображение, загружаем его превью
        if (newsData.image_path) {
          console.log("Image path detected:", newsData.image_path);
          setPreviewImage(`http://localhost:8000${newsData.image_path}`);
        } else if (newsData.image_url) {
          console.log("Image URL detected:", newsData.image_url.substring(0, 100) + "...");
          setPreviewImage(newsData.image_url);
        }
        
        setError(null);
      } catch (err) {
        console.error('Error fetching news:', err);
        setError(getErrorMessage(err, 'Ошибка при загрузке данных новости'));
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleDeleteImage = async () => {
    try {
      ...
      // Если это изображение с сервера, удаляем его
      if (formData.image_path) {
        await imagesApi.deleteNewsImage(id);
      }
    } catch (err) {
      ...
      try {
        ...
        if (selectedFile) {
          try {
            ...
            await imagesApi.uploadNewsImage(id, formData);
            console.log("Image uploaded successfully");
          } catch (err) {
            console.error("Error uploading image:", err);
            setError("Новость обновлена, но возникла ошибка при загрузке изображения");
          }
        }
        
        navigate('/admin', { state: { success: 'Новость успешно обновлена' } });
      } catch (err) {
        console.error('Error updating news:', err);
        setError(getErrorMessage(err, 'Ошибка при обновлении новости'));
      } finally {
  ...
  const getErrorMessage = (err, defaultMessage = 'Произошла ошибка') => {
    if (err.response) {
